...
По умолчанию — **замкнутый loopback-подпроцесс** (`HESTIA_RUNTIME=stub`): наш HTTP и Ed25519, опциональный `handle(payload)` только после AST-допуска. Stub **не** ставит cgroups. AST — допуск, не песочница. Дочерний процесс получает отфильтрованный env и свой cwd.

**Коробка Hestia не управляет хостом. Тенанты тоже.** Compose — один обычный спутник (нет docker CLI, нет `docker.sock`, нет privileged). Тенанты там — loopback-подпроцессы внутри коробки. Docker-runtime — процесс **на хосте** (`python -m hestia` + `HESTIA_ALLOW_HOST_DOCKER=1`), не сокет в контейнере. Стартует только `sha256:` из `HESTIA_ALLOW_IMAGE_DIGESTS`. Сеть: `hestia-tenants` только если Internal, иначе `none`. У тенантов нет `docker.sock`, нет `--privileged`, нет host-сети, `--cap-drop ALL`, `--read-only`, uid `65532`.
...
Полный гид: [user-guide.ru.md](user-guide.ru.md) · сценарии: [USE-CASES.ru.md](USE-CASES.ru.md) · архитектура: [ARCHITECTURE.md](ARCHITECTURE.md) (EN).
