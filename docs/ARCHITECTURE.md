...
HESTIA is a **single-replica control plane** plus an **invoke edge**. It hosts capability providers on the operator’s machines. It does not list them on Hub.

## Components

| Path | Responsibility |
|---|---|
| `hestia/app.py` | FastAPI: health, well-known, manifest, hearth roster, admin overview, deploy/stop/announce, SKU invoke, `/t/{slug}` proxy, `/ui/`, `/ui/admin/` |
| `hestia/config.py` | Env settings. `HESTIA_REPLICAS>1` refuses to start. `HESTIA_TENANT_NETWORK` host/bridge refused. Host `docker.sock` refused unless `HESTIA_ALLOW_HOST_DOCKER=1` |
| `hestia/docker_host.py` | Operator CLI gate: refuse host socket unless the host-process opt-in is set |
| `hestia/store.py` | SQLite tenant ledger (`hestia.db`), WAL, process-local |
...
| `hestia/runtime/docker.py` | `docker` CLI as a **host process**. Allowlisted digest only. Digest-only: no data_dir mount |
...
| `docs/landing/` | Public 3D landing (GitHub Pages) |
...
- Stub runtime does not enforce cgroups. AST admission is not a sandbox. Use docker (host process) for multi-tenant cgroup isolation.
...
- The ledger is not shared across processes. Do not put Hestia behind a multi-replica load balancer.
