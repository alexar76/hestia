...
**Воркшоп оператора (90 мин, не курс):** [workshop.ru.md](workshop.ru.md) · [EN](workshop.md) · [ES](workshop.es.md) · [FR](workshop.fr.md) · [ZH](workshop.zh.md)
...
| | Когда |
|---|---|
| SQLite (`HESTIA_DATABASE_URL` пустой) | Ноутбук, CI, `pytest`. Файл в `HESTIA_DATA_DIR`. |
| Postgres (`postgresql://…`) | Production-ledger. Отдельная база `hestia`. Флот поднимает `hestia-postgres` и пишет `hestia/.env` (`chmod 600`). |

Postgres **не** делает из очага ферму. `HESTIA_REPLICAS>1` по-прежнему отказ: тенанты — процессы или контейнеры **на этом хосте**. Липкого runtime-owner нет.

Недоверенный `handler.py` в stub — не рекламируемый production-sandbox. AST-допуск — не песочница. Для cgroup/seccomp/gVisor нужен `HESTIA_RUNTIME=docker` как процесс на хосте (`HESTIA_REQUIRE_SANDBOX=1` отказывает stub). Эталонный очаг на фабрике — stub + Postgres: production-ledger без tenant-контейнеров на factory Docker.
