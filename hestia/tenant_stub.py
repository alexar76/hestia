...
def main() -> None:
    ...
    # A tenant signs only its own result format, never a hub-verified document, so
    # it gets no ML-DSA key: that would be one more secret file in its directory.
    signer = ProviderSigner(key_path, pqc=False)
