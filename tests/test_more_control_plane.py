...
def test_apex_serves_landing_not_json_404(tmp_path) -> None:
    api = client(tmp_path)
    page = api.get("/")
    assert page.status_code == 200
    ...
    assert page.content.strip()[:1] != b"{"
    js = api.get("/hearth.js")
    assert js.status_code == 200
    assert b"probeHearth" in js.content
    fonts = api.get("/fonts/fonts.css")
    assert fonts.status_code == 200
    health = api.get("/health")
    assert health.status_code == 200
    assert health.json()["service"] == "hestia"
    roster = api.get("/v1/hearth")
    assert roster.status_code == 200
    assert roster.json()["tenants"] == []
    assert api.get("/ui/").status_code == 200
...
def test_body_too_large(tmp_path) -> None:
    ...
    res = api.post(
        "/v1/tenants",
        headers={**auth(api), "content-length": str(DEFAULT_MAX_BODY_BYTES + 1)},
        content=b"{}",
    )
...
def test_docker_start_uses_locked_argv(monkeypatch) -> None:
    ...
    # The profile's "none" is a prefix to the runtime; the tenant still gets its
    # own network, from the runtime's configured prefix.
    assert "--network hestia-tenants-safe" in joined
