...
def test_tenant_http_server(tmp_path: Path, monkeypatch) -> None:
    ...
    huge = httpx.post(
        f"{url}/invoke",
        content=b"x" * (DEFAULT_MAX_BODY_BYTES + 8),
        headers={"content-type": "application/json"},
    )
    assert huge.status_code == 413


def test_tenant_http_honors_custom_body_cap(tmp_path: Path, monkeypatch) -> None:
    cap = tmp_path / "capability.json"
    cap.write_text(
        json.dumps({"capability_id": "demo.echo@v1", "product_id": "demo"}),
        encoding="utf-8",
    ...
    monkeypatch.setenv("HESTIA_TENANT_BIND", "127.0.0.1")
    monkeypatch.setenv("HESTIA_TENANT_PORT", str(port))
    monkeypatch.setenv("HESTIA_MAX_BODY_BYTES", "1024")
    threading.Thread(target=main, daemon=True).start()
    url = f"http://127.0.0.1:{port}"
    deadline = time.time() + 3
    health = None
    ...
    assert huge.status_code == 413
    ok = httpx.post(f"{url}/invoke", json={"k": 1})
    assert ok.status_code == 200
