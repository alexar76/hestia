...
Default runtime is a **sealed loopback subprocess** (`HESTIA_RUNTIME=stub`): our HTTP server and Ed25519 signing, optional AST-admitted `handle(payload)` only. Stub does **not** enforce cgroups. AST admission is **not** a sandbox. The child gets a filtered env (no deploy token, no `AIMARKET_*`) and a per-tenant cwd.
...
- `--network hestia-tenants` only if that Docker network is `--internal`; otherwise `none`
- `--cap-drop ALL`
- `--read-only`
- `--user 65532:65532`
...
The edge at `/t/{slug}/…` proxies invoke and does not forward `Authorization`. Tenants do not get the internet from inside the box.
...
| `HESTIA_DATA_DIR` | `./data` | SQLite ledger + provider key (mode `0700`) |
| `HESTIA_RUNTIME` | `stub` | `stub` or `docker` |
...
| `HESTIA_DOCKER_TLS_VERIFY` / `HESTIA_DOCKER_CERT_PATH` | off / empty | Optional TLS for a remote engine (`DOCKER_*` honoured if unset) |
| `HESTIA_ALLOW_HOST_DOCKER` | `0` | Host-process opt-in to the host sock. Off in compose. A live sock in the box is a host shell |
| `HESTIA_TENANT_NETWORK` | `hestia-tenants` | Used only when that Docker network is Internal. `host` / `bridge` refused |
| `HESTIA_EGRESS_ALLOWLIST` | Hub hosts | Recorded only. **Not** applied to tenant argv |
| `HESTIA_REPLICAS` | `1` | `>1` refuses to start (ledger is process-local) |

## Security

See [SECURITY.md](SECURITY.md) and [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).

- Template handlers are AST-scanned (admission, not a sandbox). `os`, `subprocess`, `eval`, `open`, `getattr` / `setattr` / `type`, `operator`, dunders are refused.
- Hub / THEMIS URLs are SSRF-checked (no userinfo, no private/link-local/metadata hosts).
- Request bodies are capped. Extra JSON fields are forbidden.
- Ed25519 receipts bind result + capability + input digest.
- Single replica only until a shared store exists.
