...
The edge at `/t/{slug}/…` proxies invoke (method, body and query string) and does not forward `Authorization`. Tenants do not get the internet from inside the box. On start-up the hearth stops claiming any stub tenant left `running` by a previous process — its loopback port is no longer its own.
...
| `HESTIA_STUB_MEMORY_CAP_MB` | `0` (off) | Stub only: opt-in `RLIMIT_AS` ceiling. Low values false-kill tenants; real limits are `runtime=docker` |
